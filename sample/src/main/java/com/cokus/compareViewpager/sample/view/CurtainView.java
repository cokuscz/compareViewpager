 ...
 */
package com.cokus.compareViewpager.sample.view;
...
public class CurtainView extends FrameLayout implements ICurtainViewBase {
    ...
    private int updateMargins() {
        ...
        if(getCurtainStatus() == CurtainStatus.CLOSED) {
            if((getCurtainGravity()==CurtainGravity.LEFT&&diff<=0)
                    ||(getCurtainGravity()==CurtainGravity.RIGHT&&diff>=0))
               return 0;

            x = mFixedValue + Math.abs(diff);
        }else{
            if(diff>=0&&getCurtainGravity()==CurtainGravity.LEFT)
                return 0;
            else if(diff<=0&&getCurtainGravity()==CurtainGravity.RIGHT)
                return 0;
            x = 2*mFixedValue - Math.abs(diff);
        }
        ...
        if (mixedDiff <= 0) {
            ...
            diff = 0;
            pullingListenerOnPulling(rawStart, diff,x);
            return diff;
        }
    ...
    private void pullingListenerOnPulling(int rawStart, int diff,int x) {
        if (mOnPullingListener != null) {
            mOnPullingListener.onPulling(rawStart, diff,x, mCGravity, mCStatus);
        }
    }
...
//        Log.e("status----",getCurtainStatus().name()+mLastMotionX);
//        if(getCurtainStatus() == CurtainStatus.OPENED){
//            pullingListenerOnPulling(0, 0,mFixedValue*2);
//        }else{
//            pullingListenerOnPulling(0, 0,mFixedValue);
//        }
    }
